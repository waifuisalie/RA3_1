...
```
- **Erro Esperado:** `Operação aritmética não aceita tipo boolean`
- **Descrição:** A expressão `(5 3 >)` retorna `boolean`, negada por `!` resulta em `boolean`. A expressão completa `((5 3 >) !)` é do tipo `boolean`, que não pode ser usada no operador `+` com `10`.
- **Tipo inválido:** `boolean + int`
- **Nota:** O `!` é aplicado corretamente ao boolean `(5 3 >)`, mas o resultado ainda é boolean.
